...
export default function CalendarDayCell({
  ...
  if (!day) {
    return <div className="rounded-lg sm:rounded-xl" style={{ aspectRatio: "1/1" }} />;
  }
...
export function Icon({ type, size = 12 }: { type: Subscription["icon"]; size?: number }) {
  const sm = `sm:w-[14px] sm:h-[14px]`;
  switch (type) {
    case "music":
      return (
        <svg width={size} height={size} className={sm} viewBox="0 0 24 24" fill="currentColor">
          <path d="M9 3v10.55A4 4 0 1 0 11 17V7h4V3H9z" />
        </svg>
      );
    case "cart":
      return (
        <svg width={size} height={size} className={sm} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
